pro hheprof,cat,star,ps=ps,vsini=vrot1,vrad=vrad,resol=resol1,comp=comp, $
...
if keyword_set(obscomp) then begin
halpha,cat+'/OUT.HGAMMA'+extension,vsini=vrot,resol=resol,compmin=min(y1), $
...
!p.title=star+'H_Beta'
if keyword_set(obscomp) then begin
halpha,cat+'/OUT.HBETA'+extension,vsini=vrot,resol=resol,compmin=min(y2), $
...
if keyword_set(obscomp) then begin
halpha,cat+'/OUT.HALPHA'+extension,vsini=vrot,resol=resol,compmin=min(y3), $
...
if keyword_set(obscomp) then begin
halpha,cat+'/OUT.HEII4200'+extension,vsini=vrot,resol=resol,compmin=min(y4), $
...
if keyword_set(obscomp) then begin
halpha,cat+'/OUT.HEII4541'+extension,vsini=vrot,resol=resol,compmin=min(y5), $
...
if keyword_set(obscomp) then begin
halpha,cat+'/OUT.HEII4686'+extension,vsini=vrot,resol=resol,compmin=min(y6), $
