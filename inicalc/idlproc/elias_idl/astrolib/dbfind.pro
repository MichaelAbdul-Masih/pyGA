...
byteswap = external * (not IS_IEEE_BIG() )              ;Need to byteswap?
dbno1  = db_item_info('dbnumber', it)
byteswap  = byteswap(dbno1)
...
;
  ...
  if nlist GT 200 and not keyword_set(silent) then begin
...
;
  ...
  for i = 0l,N_left-1 do begin
        ...
        endcase
        v=v(stillgood)
        if byteswap(i) then ieee_to_host, v, idltype=idltype(i)
