; $Id: icmk_html_help.pro,v 1.6 1995/07/20 15:52:59 griz Exp $
;+
; NAME:
;	ICMK_HTML_HELP
;
...
;
 ...
 WHILE (1) DO BEGIN
  ...
  IF (found) THEN BEGIN
   ...
   ; Search for the description. It is the line following purpose.
   index = WHERE(STRTRIM(header, 2) EQ 'PURPOSE:', count)
   IF (count eq 1) THEN purp = STRTRIM(header(index(0)+1), 2) $
      ELSE purp=''
   ...
   entry = {DICT_STR2,subject:sub,indx:infile_indx,lib:libfile_indx,$
            id:ndup,offset:pos,nline:num,purpose:purp}
...
;
 ...
 PRINTF,final_file,'<!-- This file was generated by icmk_html_help.pro -->'
 ...
 PRINTF,final_file,'<CODE>icmk_html_help</CODE>.  For more information on '
 ...
 FOR i=0,count-1 DO BEGIN
  ...
  IF (entry.nline GT 0) THEN $
   PRINTF,final_file,'<LI><A HREF="#',entry.subject,'">',entry.subject,'</A>',entry.purpose
 ENDFOR
...
;----------------------------------------------------------------------------
PRO icmk_html_help, sources, outfile, VERBOSE=verbose, TITLE=title, STRICT=strict
