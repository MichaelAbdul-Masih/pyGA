pro tvarc, rmax, rmin, xc, yc, pos_ang, color, DATA = data, THICK = thick, $
	NPOINTS = npoints, COLOR = thecolor, LINESTYLE = linestyle,arc_frac=arc_frac
;+
; NAME:
;	TVARC
;
...
;
; CALLING SEQUENCE:
;	TVARC, rmax, rmin, xc, yc, [ pos_ang, color, /DATA, THICK = ,
;					 NPOINTS =, LINESTYLE  = ]
; INPUTS:
...
;
; RESTRICTIONS:
;	TVARC does not check whether the ellipse is within the boundaries
;	of the window.
;
...
;
;	TVARC does not accept normalized coordinates.
; EXAMPLE:
...
;
;	IDL> tvarc,50,30,250,100,25,thick=2
; NOTES:
;	Note that the position angle for TVARC (counter-clockwise from the
...
;-
 ...
 if not keyword_set(arc_frac) then arc_frac=1.
 if N_params() lt 2 then begin
   print,'Syntax - TVARC, rmax, rmin, xc, yc,[ pos_ang, color, COLOR = 
   print,'                         NPOINTS =, LINESTYLE = , /DATA ]'
   return
 endif
 ...
 phi = arc_frac*2*!pi*(findgen(npoints)/(npoints-1))         ;Will connect 120 points

 if keyword_set( DATA ) then begin 
		zero = 	convert_coord(0,0,/data,/to_dev) 
		coord = abs( convert_coord(rmax,0,/data,/to_dev) - zero )
		x =  coord[0]*cos(phi)
		coord = abs(convert_coord(rmin,0,/data,/to_dev) - zero)
		y =  coord[0]*sin(phi)
 endif else begin
 ...
 if keyword_set(data) then begin
	...
	xprime = round(xcen[0] + x*cosang - y*sinang)   	;Rotate to desired position angle
	yprime = round(ycen[0] + x*sinang + y*cosang)
 endif else begin
	 xprime = round(xc + x*cosang - y*sinang)   	;Rotate to desired position angle
	 yprime = round(yc + x*sinang + y*cosang)
 endelse

 plots, xprime, yprime, /DEVICE, COLOR=color, THICK=thick, LINESTYLE=linestyle
