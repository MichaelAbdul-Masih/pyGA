...
! correction for thick clumping
!          TCL = XKL*XLAMB(NB)*TCL_FAC_LINE(I)
          TCL = XKL*XLAMB(IK)*TCL_FAC_LINE(I)
!JO XLAMB CORRECTED
...
!JO: XKL needs to be modified as well 
!
...
&                INDEX1,SRVMAX,SR,NB,XNE,CLF,CLF_TEST)
!!
...
&                 VMIN,VVDOP,X,XMLOSS,XMU,YHE,DUM1
...
!      PHIKL1 = PERFIL(2,X(K),20000.d0,1.D13,FLAG)
!      PHIKL2 = PERFIL(2,X(K),40000.d0,1.D13,FLAG)
!      PHIKL3 = PERFIL(2,X(K),20000.d0,1.D14,FLAG)
!      PHIKL4 = PERFIL(2,X(K),40000.d0,1.D14,FLAG)
...
!
      PHIKL = PERFIL(1,X(K),T(L),XNE(L),FLAG)*VMAX/XLAMB(1)
      AUX=OPAL(L,1)*PHIKL      
      OPAL_TOT(K,L)=AUX 
      ETAL_TOT(K,L)=SLINE(L,1)*AUX   
      DO IK=2,NB
        PHIKL = PERFIL(IK,X(K)-XC(IK),T(L),XNE(L),FLAG)*VMAX/XLAMB(IK)
        AUX=OPAL(L,IK)*PHIKL      
...
USE nlte_dim
USE formalsol_var, ONLY: XCMFP,VZRP,WP,WP1, &
& NFCMF,XCMFE,SCONTE,SCERAY,SCE,DESCE,XNEMIN,XNUE  
