...
!doppler,nemin 
!
REAL(DP) :: VMAX,XNEMIN,VTURB
!
!----------------------------------------------------------------------
!exi 
!
INTEGER(I4B) ::  NFCMF  
REAL(DP), DIMENSION(ID_NFESC) :: XCMF
...
&                 TEM,TGRAD,VSINI,VTURB1,XCBLUE,XCRED,YHE,DUM
INTEGER(I4B) ::  I,I1,I2,I3,IESCAT,INTEG,IS,ITURB, &
&        IU,L,NB,NCH,NCO,ND,NLI,NLIN,NSUM
...
CHARACTER LEVEL(MAXNCOMP,MAXNO)*6,LEVEU(MAXNCOMP,MAXNO)*6,LINES(MAXNO)*10  

INTEGER(I4B), ALLOCATABLE, DIMENSION(:) :: NL,NU

REAL(DP), ALLOCATABLE, DIMENSION(:) :: GFLU,GL,GU,VDOP,XMAX,XMAXDOP,DELTAARR
...
PRINT *,' GIVE IN CATALOGUE NAME FOR FORMAL CALC.'  
READ (*,FMT='(A)') FILE

PRINT *,' GIVE IN VTURB. (KM/S)'  
READ (*,FMT=*) VTURB  
VTURB = VTURB*1.D5  

PRINT *,' GIVE IN IESCAT (=0, NO; = 1 WITH ESCAT)'  
...
ALL_LINES: DO NLI = 1,NLIN  
     ...
     ALLOCATE(XMAX(NCO),XMAXDOP(NCO),DELTAARR(NCO))
...
!
     CALL PREFORMAL(NCO,LEVEL(1,NLI),LEVEU(1,NLI),LINENO(1,NLI),NSTARK,VTURB,YHE)

     OPEN (1,FILE='IX.DAT',STATUS='UNKNOWN')  
     REWIND 1  
     READ (1,FMT=*) NB  
...
! 
    XNEMIN=0.
! 
    IF (ESCAT .AND. NB.NE.1) STOP &
&                 ' ELECTRON SCAT. AND DOUBLET NOT IMPLEMENTED YET'
...
!
     CALL CONTIN(XNUE,NL,NU,GFLU,SRVMAX,INDEX1,R,OPAL,OPACON, &
      ...
      XMAXDOP,TEMP,XNE,XNEFI,CLF,ESCAT)
!
...
!
     CALL FORMAL(ND,NP,NC,NB,NFOBS,RMAX,DELTA,ESCAT,R1,R,V,OPAL, &
      SLINE,P,Z,LMAX,X0,VMAX,OPALRAY,SRAY,ZRAY,LTOT, XCMF,PROFILE, &
      PROFABS,PROFEM,ZG,AIC,XMAX,XMAXDOP, OPTIOV,NSUM,OPACON, &
     ...
     DEALLOCATE(XMAX,XMAXDOP,DELTAARR)

     DEALLOCATE(OPALRAY,SRAY,XCMF)
...
&                  XMAX,XMAXDOP,TEMP,XNE,XNEFI,CLF,ESCAT)
!
! includes clumping
...
USE nlte_dim
USE fund_const, ONLY: CLIGHT
USE formalsol_var, ONLY: GAMMAL,GAMMAC,XCMF,SCONTE,NFCMF,VTURB,FILE,INVERTED  
IMPLICIT NONE
!
...
&                 XMAX(NB),XMAXDOP(NB),XNE(ND),XNEFI(ND1), &
&                 CLF(ND),XNUE(NB)
...
&                L,LI,NFRE,NREC,NRECET,NX,ITAUC1
!     ..
!     .. local arrays ..
...
! formed inside clumps, i.e., at increased densities 
     CALL CALCXMAX(XMAX(IK),XMAXDOP(IK),IK,T(DEPTHL(IK)),&
...
JLOOP: DO J = J1 + 1,J2 - 1  
          ...
          DO K = 1,NB  
               ...
               OL = OL1 + FACTOR* (OL2-OL1)  
               XNU = EXP(OL)  
               OPAL(J,K)  = C1*GFLU(K)* (XNL/GL(K)-XNU/GU(K))*SR  
...
!-----------------------------------------------------------------------
!
SUBROUTINE CALCXMAX(XMAX,XMAXDOP,IK,T,OPA,XLAMBDA,XNE,GAMMAL,GAMMAC, &
&                   OPA1,TETAUC1,XNETAUC1)  
!
USE nlte_type
USE nlte_dim
USE fund_const, ONLY: AKB,XMH=>AMH,PI,CLIGHT
USE formalsol_var, ONLY: NSTARK,WEIG,VMAX,VTURB
IMPLICIT NONE
!
...
!     ..
!     .. scalar arguments ..
REAL(DP) ::  GAMMAC,GAMMAL,OPA,T,XLAMBDA,XMAX,XMAXDOP,XNE, &
&            OPA1,TETAUC1,XNETAUC1  
INTEGER(I4B) ::  IK  
!     ..
!     .. local scalars ..
...
AUX=EPS/OPA1
XI=0.05D0
PHII=PERFIL(IK,XI,TETAUC1,XNETAUC1,FLAG)
...
USE formalsol_var, ONLY: VTURB,FILE
IMPLICIT NONE
!
...
!     ..
!     .. local scalars ..
REAL(DP) ::  A,AUX,BETA,CTECEQ,DEL,DVMIN,DVRHO,GGRAV,TEFF,VD, &
&                 VMIN,VVDOP,X,XMLOSS,XMU,YHE
...
RMAX = R(1)  
!
!---- calculation of doppler width(s) 
!
VVDOP = 1.D50  
A = SQRT(2.D0*AKB*TEFF/XMH)  
DO I = 1,NB  
     VD = A*VDOP(I)  
     VDOP(I) = SQRT(VD**2+VTURB**2)/VMAX  
...
!-----------------------------------------------------------------------
!
SUBROUTINE STAREAD(NB,NSUM,NL,NU)  
!
USE nlte_type
USE nlte_dim
...
OPEN (1,FILE='STARK.BIN',STATUS='OLD',FORM='UNFORMATTED')  
REWIND 1  
...
!
   30 CONTINUE  
PRINT *,'ERRIR IN READING STARK.BIN'  
STOP  
...
USE nlte_type
USE nlte_dim
USE fund_const, ONLY: AKB, XMH=>AMH,CLIGHT
USE formalsol_var, ONLY: WEIG,VMAX,XNEMIN,VTURB,FILE
IMPLICIT NONE
!
...
&                 CONST1,CORRFC,DBDR,DELDO,DELTAX,DX,DX1,DX2,DX3, &
&                 DX4,ERR,GGRAV,QBLUE,QRED,REL,SCNEW,SCOLD,SR, &
&                 SRVMAX,TEFF,VTH,VTHEL,WEIGHT,X1,X2,XCBLUE, &
&                 XCRED,XKL,XLAMBDA,XMAX,XMLOSS,XMU,XNL,XNU,XNUE0, &
&                 XXX,YHE,Z2
INTEGER(I4B) ::  I,IIT,IK,J,JP,K,K0,K1,L,LMAX,LZ,NC,NF,NF1,NF2,NF3,NF4, &
&        NFCMF,NFRE,NREC,NRECET
LOGICAL FLAG  
!     ..
!     .. local arrays ..
REAL(DP) ::  AICK(NFMAX),AJ(NFMAX,ND),BLEVEL(ID_LLEVS), &
&                 DBDRK(NFMAX),DVDR(ND),E(NFMAX,ND), &
&                 ENIONND(KEL,KIS+1,ND),EOLD(NFMAX),ETAC(NFMAX,ND), &
&                 FI(NFMAX),FREQC(IFRETOT),OPAC(ND), &
&                 OPACON(ND,IFRETOT),P(NP), &
&                 PHI(NFMAX,ND),R(ND),SCTOT(ND), &
&                 ST(ND,IFRETOT),STRUE(ND),T(ND),THOMS(ND,IFRETOT), &
&                 THOMSON(ND),UBLUWI(ND,NP-1),UCMF(ND), &
&                 V(ND),VBLUWI(ND-1,NP-1),VCMF(ND-1),W0(ND,NP-1), &
&                 W2(ND,NP-1),X(NFMAX),XI(NFMAX),XJALL(ND,IFRETOT), &
&                 XJNEW(NFMAX,ND),XJOLD(ND),XNE(ND),DUM(ND),CLF(ND),&
&                 Z(ND,NP)

! these arrays need to be modified if NB ne 1
REAL(DP) :: OPAL(ND,1),SLINE(ND,1),XLAMB(1)
!     ..
!     .. external functions ..
REAL(DP) ::  PERFIL  
EXTERNAL PERFIL  
...
IF (NB.NE.1) STOP 'NB NE 1 IN ELSCAT'  
...
END DO  
!
!     up to now linear interpolation at line freq. to be consistent with
!     treatment in formacon
!
DX = XCBLUE - XCRED  
...
DO I = 1,ND  
     ...
     READ (1,REC=I) (BLEVEL(J),J=1,NREC)  
     OPAC(I) = (QRED*OPACON(I,K0)+QBLUE*OPACON(I,K0+1))*SR  
     THOMSON(I) = QRED*THOMS(I,K0) + QBLUE*THOMS(I,K0+1)  
!
!--------possibility that thoms > 1 due to updated electr. densities
!        in nlte code.
!
     IF (THOMSON(I).GT.1.D0) THOMSON(I) = 1.D0  
     STRUE(I) = QRED*ST(I,K0) + QBLUE*ST(I,K0+1)  
     XJOLD(I) = QRED*XJALL(I,K0) + QBLUE*XJALL(I,K0+1)  
...
!     for calculation vth = vth(teff, min(atomic weight)
!     assumed
!     calculation in four freq. bands:
!     xmax+x1....x1          xmax=3*vtherm(el)
!          x1....-x1           x1=4*vth
!         -x1....-x2           x2=2*vmax
!         -x2....-x2-xmax
!
...
DO IK = 1,NB  
     WEIGHT = MIN(WEIGHT,WEIG(IK))  
END DO  

CONST = 2.D0*AKB/ (XMH*WEIGHT)  
CONST1 = 1.D0/CONST  
VTH = SQRT(CONST*TEFF+VTURB**2)  
!
!---  assumes that turbulence velocity if negligible for electrons
!
...
IF (X2.LT.X1) STOP ' 2*VINF < 4*VTH, CHANGE FREQ. GRID!'  
!
!     four points per vtherm
!
X(1) = XMAX + X1  
NF1 = 3*4  
DX1 = XMAX/DBLE(NF1)  
...
!     10 points per vmax
!
NF3 = INT((2.D0-VTH/VMAX)*10.D0)  
IF (MOD(NF3,2).NE.0) NF3 = NF3 + 1  
...
!
!=======================================
!     cmf solution for first freq. range
!=======================================
!
     DELTAX = DX1*DELDO  
     ...
     DO L = 1,ND  
          DO K = 1,NF1 + 1  
               ...
               IF (IIT.EQ.1) THEN  
                    SCTOT(L) = STRUE(L) + THOMSON(L)*XJNEW(K,L)  
               ELSE  
                    SCTOT(L) = STRUE(L) + THOMSON(L)*E(K,L)  
               END IF  

               ETAC(K,L) = SCTOT(L)*OPAC(L)  
               PHI(K,L) = PERFIL(1,X(K1),T(L),XNE(L),FLAG)* &
                VMAX/XLAMB(1)
               AJ(K,L) = 0.D0  
...
!
          ...
          CALL RAY(JP,Z,R,V,DVDR,ND,NP,UCMF,VCMF,SLINE(1,1),PHI, &
           NF1+1,DELTAX,OPAC,ETAC,OPAL(1,1),AICK,DBDRK,AJ, W0(1,JP))

          UBLUWI(1,JP) = UCMF(1)  

          DO L = 1,LZ  
               UBLUWI(L+1,JP) = UCMF(L+1)  
               VBLUWI(L,JP) = VCMF(L)  
          END DO
...
!
!=======================================
!     cmf solution for 2nd freq. range
!=======================================
!
     DELTAX = DX2*DELDO  
     ...
     DO L = 1,ND  
          DO K = 1,NF2 + 1  
               ...
               IF (IIT.EQ.1) THEN  
                    SCTOT(L) = STRUE(L) + THOMSON(L)*XJNEW(K1,L)  
               ELSE  
                    SCTOT(L) = STRUE(L) + THOMSON(L)*E(K1,L)  
               END IF  
...
!
               PHI(K,L) = PERFIL(1,X(K1),T(L),XNE(L),FLAG)* &
                VMAX/XLAMB(1)
               AJ(K,L) = 0.D0  
               XLAMBDA = XLAMB(1)*1.D8/ (VMAX*X(K1)/CLIGHT+1.D0)  
               CALL DIFFUS(XLAMBDA,T,R,ND,AICK(K),DBDRK(K))  
          END DO
     END DO
...
JPLOOP2: DO JP = 1,NP - 1  
          ...
          LZ = LMAX - 1  
!
!     bluewing boundary condition: u,v (blue) = u,v(red) from first rang
!
          UCMF(1) = UBLUWI(1,JP)  
          DO L = 1,LZ  
               UCMF(L+1) = UBLUWI(L+1,JP)  
          ...
          CALL RAY(JP,Z,R,V,DVDR,ND,NP,UCMF,VCMF,SLINE(1,1),PHI, &
           NF2+1,DELTAX,OPAC,ETAC,OPAL(1,1),AICK,DBDRK,AJ, W0(1,JP))
          UBLUWI(1,JP) = UCMF(1)  
          DO L = 1,LZ  
               UBLUWI(L+1,JP) = UCMF(L+1)  
               VBLUWI(L,JP) = VCMF(L)  
     ...
     DO K = 1,NF2 + 1  
          ...
          ERR = 0.D0  
          DO L = 1,ND  
!      print*,k,' ',x(k1),' ',l,' ',aj(k,l)/xjold(l)
               ERR = MAX(ERR,ABS(1.D0-AJ(K,L)/XJOLD(L)))  
               XJNEW(K1,L) = AJ(K,L)  
          END DO
!      print*
!      print*,' max. inconsistency in jnue: ',err,' at x(',k1,') = ',x(k1)
     END DO
!
!=======================================
!     cmf solution for 3rd freq. range
!=======================================
!
     DELTAX = DX3*DELDO  
!
...
JPLOOP3: DO JP = 1,NP - 1  
          LMAX = MIN0(NP+1-JP,ND)  
          LZ = LMAX - 1  
!
!     bluewing boundary condition: u,v (blue) = u,v(red) from 2nd range
!
          UCMF(1) = UBLUWI(1,JP)  
          DO L = 1,LZ  
               UCMF(L+1) = UBLUWI(L+1,JP)  
          ...
          CALL RAY(JP,Z,R,V,DVDR,ND,NP,UCMF,VCMF,SLINE(1,1),PHI, &
           NF3+1,DELTAX,OPAC,ETAC,OPAL(1,1),AICK,DBDRK,AJ, W0(1,JP))
           UBLUWI(1,JP) = UCMF(1)
...
!
!=======================================
!     cmf solution for 4th freq. range
!=======================================
!
     DELTAX = DX4*DELDO  
...
JPLOOP4: DO JP = 1,NP - 1  
          LMAX = MIN0(NP+1-JP,ND)  
          LZ = LMAX - 1  
!
!     bluewing boundary condition: u,v (blue) = u,v(red) from 3rd range
!
          UCMF(1) = UBLUWI(1,JP)  
          DO L = 1,LZ  
               UCMF(L+1) = UBLUWI(L+1,JP)  
          ...
          CALL RAY(JP,Z,R,V,DVDR,ND,NP,UCMF,VCMF,SLINE(1,1),PHI, &
           NF4+1,DELTAX,OPAC,ETAC,OPAL(1,1),AICK,DBDRK,AJ, W0(1,JP))
           UBLUWI(1,JP) = UCMF(1)
...
LLOOP: DO L = 1,ND  
          ...
          CONST = (BETAT/B1)**2  
          CALL SCATSOL(XI,XJNEW(1,L),FI,CONST,NFCMF)  
...
!-----------------------------------------------------------------------
!
SUBROUTINE RAY(JP,Z,R,VELO,GRADV,ND,NP,U,V,SLINE,PHI,NF,DELTAX, &
&               OPA,ETA,OPAL,AIC,DBDR,AJ,W0)
!
USE nlte_type
USE nlte_dim
IMPLICIT NONE
...
!
!     .. parameters ..
INTEGER(I4B), PARAMETER :: ND1=ID_NDEPT,NFMAX=ID_NFESC  
!     ..
!     .. scalar arguments ..
REAL(DP) ::  DELTAX  
...
!     ..
!     .. array arguments ..
REAL(DP) ::  AIC(NFMAX),AJ(NFMAX,ND),DBDR(NFMAX), &
&                 ETA(NFMAX,ND),GRADV(ND),OPA(ND),OPAL(ND), &
&                 PHI(NFMAX,ND),R(ND),SLINE(ND),U(ND),V(ND-1), &
&                 VELO(ND),W0(ND),Z(ND,NP)
!     ..
!     .. local scalars ..
REAL(DP) ::  RL,XMUE,XMUE2  
INTEGER(I4B) ::  K,L,LMAX,LZ  
!     ..
!     .. local arrays ..
REAL(DP) ::  GA(ND1),H(ND1),PP(ND1),QQ(ND1),S(ND1),TA(ND1), &
&                 TB(ND1),TC(ND1),UB(ND1,3),UOLD(ND1),VA(ND1), &
&                 VB(ND1)
!     ..
!     .. external subroutines ..
...
DO L = 1,LMAX  
     ...
     XMUE = Z(L,JP)/RL  
     XMUE2 = XMUE*XMUE  
     PP(L) = (XMUE2*GRADV(L)+ (1.D0-XMUE2)*VELO(L)/RL)/DELTAX  
END DO  
!
...
KLOOP: DO K = 1,NF  
     ...
     CALL CMFSET(K,PHI,Z(1,JP),ND,LMAX,TA,TB,TC,UB,VA,VB,GA,H, &
      SLINE,S,OPA,ETA,OPAL,PP,DBDR,AIC)
     ...
     DO L = 2,LMAX - 1  
          UOLD(L) = UB(L,1)*U(L-1) + UB(L,2)*U(L) + UB(L,3)*U(L+1)  
     END DO

     UOLD(LMAX) = UB(LMAX,2)*U(LMAX)  
     U(1:LMAX)=UOLD(1:LMAX)
     CALL VADD(U,QQ,LMAX)  
...
!-----------------------------------------------------------------------
!
SUBROUTINE CMFSET(K,PHI,Z,ND,LMAX,TA,TB,TC,UB,VA,VB,GA,H,SLINE,S, &
&                  OPA,ETA,OPAL,PP,DBDR,AIC)
!
USE nlte_type
USE nlte_dim
IMPLICIT NONE
...
!
!     .. parameters ..
INTEGER(I4B), PARAMETER :: NFMAX=ID_NFESC  
!     ..
!     .. scalar arguments ..
INTEGER(I4B) ::  K,LMAX,ND  
!     ..
!     .. array arguments ..
REAL(DP) ::  AIC(NFMAX),DBDR(NFMAX),ETA(NFMAX,ND),GA(ND), &
&                 H(ND),OPA(ND),OPAL(ND),PHI(NFMAX,ND),PP(ND), &
&                 S(ND),SLINE(ND),TA(ND),TB(ND),TC(ND),UB(ND,3), &
&                 VA(ND),VB(ND),Z(ND)
!     ..
!     .. local scalars ..
REAL(DP) ::  AK,AZ,DAZ,DAZM,DBZ,DBZM,DT,DTZ,DTZM,DX,DXZ,DXZM, &
&                 EK,TAU,TAUZ,UB1,UB2,UB3
INTEGER(I4B) ::  L,LZ  
!     ..
!     .. intrinsic functions ..
INTRINSIC ABS  
...
!
AK = PHI(K,1)*OPAL(1) + OPA(1)  
AZ = 0.5D0* (AK+PHI(K,2)*OPAL(2)+OPA(2))  
TAUZ = Z(1) - Z(2)  
DX = PP(1)/AK  
S(1) = 0.D0  
TC(1) = 1.D0/AK/TAUZ  
...
!
DO L = 2,LZ  
     AK = PHI(K,L)*OPAL(L) + OPA(L)  
     EK = SLINE(L)*OPAL(L)*PHI(K,L) + ETA(K,L)  
     S(L) = EK/AK  
     AZ = 0.5D0* (AK+PHI(K,L+1)*OPAL(L+1)+OPA(L+1))  
...
!      ub3=-hp/hs
     ...
     UB2 = 1.D0  
     UB3 = 0.D0  
     IF (ABS(UB1+UB2+UB3-1.D0).GT.1.D-8) STOP ' ERROR IN UB'  
     TA(L) = DT*DAZM  
     TC(L) = DT*DAZ  
     TB(L) = TA(L) + TC(L) + (DX+1.D0)*UB2  
     TA(L) = TA(L) - (DX+1.D0)*UB1  
     TC(L) = TC(L) - (DX+1.D0)*UB3  
     UB(L,1) = DX*UB1  
     UB(L,2) = DX*UB2  
     UB(L,3) = DX*UB3  
     VA(L) = -DT*DBZM  
...
DT = 1.D0/AK/TAUZ  
DX = PP(L)/AK  
TA(L) = 2.D0*DT*DTZM/ (1.D0+DXZM) !corrected  
TB(L) = TA(L) + DX + 1.D0  
UB(L,2) = DX  
VA(L) = -2.D0*DT*DXZM/ (1.D0+DXZM)  !corrected
...
!-----------------------------------------------------------------------
!
SUBROUTINE SCATSOL(XI,XJ,FI,CONST,NF)  
!
USE nlte_type
USE nlte_dim
IMPLICIT NONE
...
!     ..
!     .. local arrays ..
REAL(DP) ::  TA(NFMAX),TB(NFMAX),TC(NFMAX)  
!     ..
!     .. external subroutines ..
EXTERNAL INVTRI  
!     ..

IF (NF.GT.NFMAX) STOP ' NF > NFMAX IN SCATSOL'  
!
!     first frequency
!
DDP = XI(2) - XI(1)  
...
&                  SLINE,P,Z,LMAX,X0,VMAX,OPALRAY,SRAY,ZRAY,LTOT, &
&                  XCMF,PROFILE,PROFABS,PROFEM,ZG,AIC,XMAX,XMAXDOP, &
...
USE formalsol_var, ONLY: XNEMIN,VTURB,INVERTED,FILE
IMPLICIT NONE
!
!
!     .. parameters ..
...
&                 SLINE(NDM,NB),SRAY(LTO1,NB),TEMP(NDM),V(NDM), &
&                 VDOP(NB),X0(NF),XCMF(LTO1,NB),XMAX(NB),XMAXDOP(NB), &
...
CALL XGRID(DELTA,NFOBS,XMAXDOP,X0,AIC,OPACON,ND,TEM,TGRAD,XCRED, &
...
!     KLOOP: DO K = 34,34  
          ...
          CALL PREPRAY(Z(1,JP),P,NP,NB,JP,X0(K),LTOT,LM,CORE,W,W1, &
           V, VVDOP,R,OPAL,SLINE,OPALRAY,SRAY,ZRAY,XCMF, DELTA, &
...
!
                    CALL OBSFRAM1(LM1,NB,CORE,EMINT,OPALRAY,SRAY, &
                     ZRAY, XCMF,IZR,IZB,EMINT1,ZG(JP,K),TAUK, &
...
!
               ELSE  
                    CALL OBSFRAM(LM1,NB,CORE,EMINT,OPALRAY,SRAY, &
                     ZRAY, XCMF,IZR,IZB,IZNER,IZNEB,EMINT1, &
...
&                ' SOMETHING WRONG WITH XMAX FOR DOPPLER-DOUBLETS!'
               ...
               CALL OBSFRAM2(LM1,NB,CORE,EMINT,OPALRAY,SRAY,ZRAY, &
...
!-----------------------------------------------------------------------
!
SUBROUTINE OBSFRAM(LM,NB,CORE,EMINT,OPALRAY,SRAY,ZRAY,XCMF,IZR, &
...
!     ..
!     .. external functions ..
REAL(DP) ::  EXPUNO,PERFIL  
EXTERNAL EXPUNO,PERFIL  
!     ..
!     .. intrinsic functions ..
...
DO K = 1,NB  
     XI = XCMF(L,K)  
     PHII = PERFIL(K,XI,TEMPRAY(L),XNERAY(L),FLAG)  
     IF (FLAG) CYCLE  
...
!-----------------------------------------------------------------------
!
SUBROUTINE OBSFRAM1(LM,NB,CORE,EMINT,OPALRAY,SRAY,ZRAY,XCMF,IZR, &
...
!-----------------------------------------------------------------------
!
SUBROUTINE OBSFRAM2(LM,NB,CORE,EMINT,OPALRAY,SRAY,ZRAY,XCMF,IZR, &
...
&                   OPAL,SOURCE,OPALRAY,SRAY,ZRAY,XCMF,DELTA,XMAX, &
...
USE nlte_dim
USE formalsol_var, ONLY: XCMFP,VZRP,WP,WP1, &
& NFCMF,XCMFE=>XCMF,SCONTE,SCERAY,SCE,DESCE,XNEMIN,XNUE  
...
!     ..
!     .. local scalars ..
REAL(DP) ::  DELLIN,DELOPA1,DELS1,DELT1,DELT2, &
...
&                 SUMEX,T1,T2,VDOPMAX,VDOPMIN,VZR,VZR1,XCMFL, &
&                 XMAXI,XN1,Z1,ZI
...
!-----------------------------------------------------------------------
!
SUBROUTINE XGRID(DELTA,NFOBS,XMAXDOP,X0,AIC,OPACON,ND,TEM,TGRAD, &
&                 XCRED,XCBLUE,XNUE0,VMAX,NB,VPLUS,ESCAT)
!
USE nlte_type
USE nlte_dim
USE fund_const, ONLY: CLIGHT
USE formalsol_var, ONLY: NFCMF,XCMF,XREST=>XNUE
IMPLICIT NONE
!
!
!     .. parameters ..
...
IF (.NOT.ESCAT) XCMF(1) = 0.D0  

IF(NB.GT.2) GOTO 200

! path for singlets and doublets

IF (.NOT.ESCAT .OR. FMAX.GE.XCMF(1)) THEN  
     M = NFOBS/3  
!
!---- blue wing
!
...
ELSE  ! ELSCAT TREATMENT, ONLY ONE COMPONENT ALLOWED, THUS NO DIFFERENCE

     IF (NB .NE. 1) STOP' ELSCAT AND TWO COMPONENTS IN XGRID!'
  
     IF (XCMF(1)-XCMF(2).LT.1.) STOP ' DELTA X(1) < 1'  

     IEB = 0  

     DO K = 1,NFCMF  
          IF (XCMF(K).GT.FMAX) IEB = IEB + 1  
     END DO

     IER = 0  

     DO K = 1,NFCMF  
          IF (XCMF(K).LT.-FMIN) IER = IER + 1  
     END DO
...
!
     DO I = 1,IER - 1  
          K = K + 1  
          X0(K) = XCMF(NFCMF-IER+I)  
     END DO

     K = K + 1  
     X0(K) = XCMF(NFCMF) + 1.D0  
...
!***********************************************************************
!
FUNCTION PERFIL(I,XI,TEM,XNE,FLAG)  
!
...
USE formalsol_var, ONLY: GAMMAL,GAMMAC,VMAX,XNEMIN,VTURB,WEIG,XNUE, &
...
!
!--- pure doppler profile (normalized in frequencies)
!
     AKTM = 2.D0*AKB*TEM/XMH/WEIG(I)+VTURB*VTURB  
     ...
     IF (A.GT.25) THEN  
          ...
          FLAG = .TRUE.  
     ELSE  
          PERFIL = EXP(-A)/SQRT(AKTM)*XLAMB0/RAPI  
     END IF  
...
!------- voigt profile
!
     VTH = SQRT(2.D0*AKB*TEM/XMH/WEIG(I)+VTURB*VTURB)  
     DELDOP = VTH*XNUE(I)  
!
...
!
     ...
     XD = XI*VMAX/VTH  
     PERFIL = VOIGT(AVOIGT,XD)*XLAMB0/ (VTH*RAPI)  
...
!
     ...
     IF (ELOG.LT.ES(1,I)) THEN
      IF(XNEMIN.NE.0.) STOP ' ERROR IN XNEMIN OR STARK PROFILE TABLES'
!
...
!
     ...
     PERFIL = 10.D0** (WEF* (PF-PL)+PL)  

ELSE  

    STOP 'ERROR IN NSTARK  -  PERFIL'  
