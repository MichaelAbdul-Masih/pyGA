...
def crossover_and_mutate_raw(population_raw, fitness, parameters, mutation_rate):
    """
    ...
    creep_pop = gaussian_creep_mutation_raw_all(mutated_pop, parameters, mutation_rate)
    return creep_pop
...
def gaussian_creep_mutation_raw_all(mutated_pop, parameters, mutation_rate = 0.25):
    """
    """
    '''Must provide chromosomes for a population after crossover and random mutation, the parameters dictionary, and a mutation rate.'''
    ...
    creep_chance = np.random.rand(len(mutated_pop), len(keys))

    for individual_number in range(len(creep_pop)):
        chromosome = creep_pop[individual_number]
        start_ind = 0
        for i, key in enumerate(keys):
            precision = parameters[key].precision
            if creep_chance[individual_number][i] <= mutation_rate:
                gene = creep_pop[individual_number][start_ind:start_ind+precision]
                perturbed_gene = abs(float(gene)/10**precision + np.random.normal(0, 0.1) + np.random.normal(0, 0.025))
                perturbed_gene -= perturbed_gene % 1 * 2 * (perturbed_gene > 1)
                new_gene_string = str(round(perturbed_gene, precision))[2:].ljust(precision, '0')
                chromosome = chromosome[:start_ind] + new_gene_string + chromosome[start_ind+precision:]
            start_ind += precision
        creep_pop[individual_number] = str(chromosome)
    return creep_pop
#Returns the chromosomes for a population after the creep mutation has been applied.
