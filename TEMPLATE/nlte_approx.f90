...
character*5 :: ver_nlte_app='7.4.0'
...
! levels, leading to quite high rations (100) and partition functions (10^4).
              endif
...
do ll=1,lwion-1
     ...
     do k=1,natom
        ...
        irat=irat+1
     enddo
     if(irat.ne.sum(jatom_full)) stop' error in irat'
...
USE nlte_porvor, ONLY: epsi1,opa_eff_rat, opa_eff_rat_old,epsi2
!JS-CHANGED -- compiler issues
...
&               ndr,icompdr,nlcounter,indicator,itrans,l
...
& ener,flu,xlamc,afac,add,x1a,beta_dum
!JS-CHANGE
...
!JS-CHANGE July 2015: Due to (strange) numerical precision issue
!in new intel fortran copiler (2015 version ofr mac)
      if(ll.eq.louter(k) .and. ind.ne.0 .and. indicator.ne.3) then
         if (abs(betacic-jbar_bg(nlcounter,ll)).gt.epsi2) then
            print*,k,j,ii,ind,lam,betacic,jbar_bg(nlcounter,ll)
            stop' error in betacic'
         endif
         beta_dum = 1.d0-beta
         if (abs(beta_dum-alo_bg(nlcounter,ll)).gt.epsi2) then
